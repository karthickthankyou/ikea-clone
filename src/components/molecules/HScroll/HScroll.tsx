...
const Arrow = ({
  ...
  return (
    ...
    >
      {children || (
        <div
          className={`w-10 h-10 p-2 bg-black text-white rounded-full ${arrowClassName}`}
        >
...
const HScrollBody = ({ children, className }: HScrollBodyProps) => {
  ...
  return (
    <div
      ...
      className={`flex w-full thin-scrollbar py-3 space-x-2 overflow-x-scroll snap-x snap-mandatory scrollbar-hide ${className}`}
    >
      {children}
    </div>
  )
